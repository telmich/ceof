...
"`\textit{0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ-!}"'.
The length of an EOFID is 6 bytes, which results in
\emph{68719476736} possible ids.\footnote{$(26+26+10+2)^6$}.
The characters where selected allow easy debugging.
% -----------------------------------------------------------------------------
...
% -----------------------------------------------------------------------------
\subsection{Size}
A size is is always represented as an ASCII number in a fixed length
string of 6 bytes.
\subsubsection{Examples}
\begin{verbatim}
>>> import ceof
...
\subsubsection{Examples}
\begin{verbatim}
% LC_ALL=C gpg --fingerprint | grep "Key fingerprint =" | sed -e 's/.*=//' -e 's/ //g'
